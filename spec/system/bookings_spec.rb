...
RSpec.describe 'Bookings', type: :system do
  ...
  describe 'creating a booking' do
    ...
    it 'prevents booking completed flights' do
      completed_flight = create(:flight)
      completed_flight.update_columns(
        status: :completed,
        departure_time: 1.day.from_now,
        estimated_arrival_time: 2.days.from_now
      )
      
      visit flight_path(completed_flight)
      expect(page).not_to have_link('Book This Flight')
      expect(page).to have_content('This flight is no longer available for booking')
    end
  end
  ...
  describe 'managing a booking' do
    ...
    context 'with a cancelled booking' do
      let!(:booking) { create(:booking, :cancelled, user: passenger) }

      it 'shows cancelled status without action buttons' do
        visit booking_path(booking)
        expect(page).to have_content('Status: Cancelled')
        expect(page).not_to have_button('Confirm Booking')
        expect(page).not_to have_button('Cancel Booking')
